...
if create_dataset:
    ...
    for face in test_set:
        ...
        save_path = str(Path(test_dir) / '1' / Path(face['face']).stem ) +  '.png'
        img1.save( save_path, 'PNG')
        face['1'] = save_path
        img0 = random_crop(face['image'],resolution)
        img0 = img0.convert('L') # convert to grayscale
        save_path = str(Path(test_dir) / '0'/ Path(face['image']).stem ) +  '.png'
    ...
    for face in training_set:
        ...
        img1 = img1.convert('L') # convert to grayscale
        img1 = img1.resize(resolution)
        save_path = str(Path(training_dir) / '1'/ Path(face['face']).stem) +  '.png'
        img1.save( save_path, 'PNG')
        face['1'] = save_path
        img0 = random_crop(face['image'],resolution)
        img0 = img0.convert('L') # convert to grayscale
        save_path = str(Path(training_dir) / '0'/ Path(face['image']).stem ) +  '.png'
        img0.save( save_path, 'PNG')
        face['0'] = save_path
